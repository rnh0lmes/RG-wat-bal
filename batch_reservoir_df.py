...
#***********************LOAD INPUT***********************#
# Load Excel File
# fileName = 'C:\\Users\\robyn\\Documents\\Grad_School\\Research\\swim-scilab\\Python\\Inputs\\input_parameters_all.xlsx'    #filename
fileName = 'Inputs\\input_parameters_all.xlsx'
...
EvapCoeffA = Scalars.at['EvapCoeff', 'Value']           # Evaporation Coefficient A
...
print(Net_Local.loc[2021:2070,:].melt().describe())
...
def perChangePlt(xVar_df, xVar_name, yVar_df, yVar_name, plot_title, histStartYr=1971, histEndYr=2020, futStartYr=2021, futEndYr=2070):
    ...
    plt.figure()
    pcp = sns.scatterplot(data=EB_delta, x=xVar_name, y=yVar_name,  palette=['#01665e'])

    # plt.xlabel(xVar_name)
    # plt.ylabel(yVar_name)
    # plt.title(plot_title)

    Ystats = EB_delta[yVar_name].describe()
    Xstats = EB_delta[xVar_name].describe()
    print('Descrobe' + yVar_name)
    print(Ystats)
    print('Describe' + xVar_name)
    print(Xstats)
    
    #pcp.axhline(Ystats['25%'], color='tab:gray', linestyle='dashed')
    pcp.axhline(Ystats['mean'], color='tab:gray',linestyle='dashed')
    #pcp.axhline(Ystats['75%'], color='tab:gray', linestyle='dashed')
    #pcp.axvline(Xstats['25%'], color='tab:grey', linestyle='dashed')
    pcp.axvline(Xstats['mean'], color='tab:grey',linestyle='dashed')
    #pcp.axvline(Xstats['75%'], color='tab:grey', linestyle='dashed')
    pcp.axhline(0)
    pcp.axvline(0)
    x = np.linspace(-35,35)
    y = -x
...
def cdf(df,histStart=1971,histEnd=2020,futStart=2021,futEnd=2070):
 
    hist = df.loc[histStart:histEnd,:].melt()
    fut = df.loc[futStart:futEnd,:].melt()
    n = len(hist)
           
    for frame in [hist,fut]:
        frame = frame.sort_values(by=['value'])
        frame = frame.reset_index()
        frame = frame.drop(columns=['index','variable'])
        frame = frame.reset_index()
        frame = frame.rename(columns={'index':'rank'})
        frame['cdf'] = frame['rank'] / (n+1)
        plt.plot(frame['value'],frame['cdf'])
    plt.legend(['past','future'])
    plt.xlim(0,10000000)

cdf(Cab_Release_af)

def cdfmany(df,histStart=1971,histEnd=2020):#,futStart=2021,futEnd=2070):
 
    df = df.loc[histStart:histEnd,:]
    ...
    for col in df.columns.tolist():
        ...
        column = column.rename(columns = {col:'value'})
        column = column.sort_values(by=['value'])
        column = column.reset_index()
        column = column.drop(columns=['index'])
        column = column.reset_index()
        column = column.rename(columns={'index':'rank'})
        column['cdf'] = column['rank'] / (n+1)
        plt.plot(column['value'],column['cdf'])
    plt.figure()
    plt.legend(['past','future'])
    plt.xlim(0,10000000)

cdfmany(Cab_Release_af)
cdfmany(Cab_Release_af, 2021,2070)
...
pctOverTimePlt(Res_SEvap_af,'surface evap','year','af')
pctOverTimePlt(Res_SPrecip_af, 'precip', 'year', 'af')
pctOverTimePlt(Cab_RO_af, 'runoff', 'year', 'af')
pctOverTimePlt(Cab_Release_af,'caballo release', 'year','af')
pctOverTimePlt(Cab_Gauge_af,'cab gauge (max 850kaf','year','af')

#Cab Hisogram
def releaseHisto(release_df,startYear=2021,endYear=2070,pctLow=.1,pctHigh=.9):
    release_df = release_df.loc[startYear:endYear,:]
    release_df = release_df.mean()
    plt.figure()
    release_df.plot.hist()
    release_df = release_df.describe([pctLow,.5,pctHigh])
    print(release_df)
    
    pctLowTxt = str(int(100*pctLow))+"%"
    pctHighTxt = str(int(100*pctHigh))+'%'
    plt.axvline(release_df[pctLowTxt])
    plt.axvline(release_df['50%'])
    plt.axvline(release_df[pctHighTxt])
# releaseHisto(Cab_Release_af)
# releaseHisto(Cab_Release_af,pctLow=.05,pctHigh=.95)
# releaseHisto(Cab_Release_af,pctLow=.25,pctHigh=.75)

releaseHisto(SanMar_af,startYear=1971,endYear=2020)
releaseHisto(SanMar_af,startYear=1971,endYear=2020,pctLow=.05,pctHigh=.95)
releaseHisto(SanMar_af,startYear=1971,endYear=2020,pctLow=.25,pctHigh=.75)
releaseHisto(SanMar_af)
releaseHisto(SanMar_af,pctLow=.05,pctHigh=.95)
releaseHisto(SanMar_af,pctLow=.25,pctHigh=.75)
# Caballo Temperature
...
#WBplotScns = ['access1_0_rcp45']     
#watBalPlt(WBplotScns,EB_Storage_af,Res_SPrecip_af,Res_SEvap_af,Cab_RO_af,SanMar_af,Cab_Release_af, 2021, 2070) 
#inOutAreaPlts(WBplotScns,Res_SPrecip_af,Res_SEvap_af,Cab_RO_af,SanMar_af,Cab_Release_af, 2021, 2070)
...
pchangevars=[EBTas_degC,EBPr_ft,SanMar_af,Cab_Release_kaf,Cab_Gauge_af]
...
perChangePlt(Net_Local, 'Change in Net Local Reservoir Contributions (kaf/yr)', EB_SurfA_ac, 'Change in Upstream Reservoir Contributions (kaf/yr)', 'Local vs Upstream Change 1971-2020 vs 2021-2070', 1971, 2020, 2021, 2070)
